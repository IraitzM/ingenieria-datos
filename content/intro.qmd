...
Para ello, deberemos hacernos eco del ciclo de vida de los datos en toda su extensión. Aunque nos centraremos mucho en los conceptos y las herramientas serán ejemplos en los que podemos implementar estas ideas.
...
de las máquinas de escribir y que esta configuración a pesar de no ser la que nos permite una mayor velocidad de redacción impedía que los martillos de la máquina se bloquearan.

![Martillos en una máquina de escribir](../assets/maquinaescribir.webp){width=60%}

La misma razón por la que la tecla enter en sus inicios se conocía como retorno de carro (_return carriage_) cuando no existe carro que retornar en nuestros ordenadores modernos.
...
[International Business Machines](https://es.wikipedia.org/wiki/IBM) (IBM) fue una de las primeras compañías en realizar esa transición de máquinas que habían servido para fines militares al ámbito comercial, corporativo, al servicio de instituciones privadas con problemas de grandes dimensiones.
...
Son los que conocemos como [sistemas centrales de datos](sistemas/intro) y gran parte de este trabajo se lo debemos a una persona en concreto, [Edgar Frank Codd](https://es.wikipedia.org/wiki/Edgar_Frank_Codd), y su trabajo en este ámbito titulado _Un modelo tabular[^1] de datos para grandes bancos de datos compartidos_ [@codd1970relational]. Para quien quiera acompañar este libro con dos lecturas de referencia, _Designing Data-Intensive Applications_ [@kleppmann2017designing] cubre los fundamentos de los sistemas que hay debajo, y _Fundamentals of Data Engineering_ [@reis2022fundamentals] el oficio en su conjunto.

:::{.callout-note}
# Cómo leer los diagramas

A lo largo del libro los diagramas siguen siempre el mismo código de color, que se corresponde con las tres grandes partes del recorrido del dato. Merece la pena echarle un vistazo ahora para no tener que descifrarlo sobre la marcha.

```{mermaid}
flowchart LR
    o["Sistemas origen"] --> c["Ingesta"] --> t["Transformación"] --> e["Explotación"]
    m["Metadatos"] -.-> t
    a[("Almacenamiento")] -.-> t
...
En gris lo que no controlamos, los sistemas de los que bebemos. En ámbar la **ingesta**, en verde la **transformación** y en violeta la **explotación**, que son las tres partes centrales del libro. En azul el almacenamiento y en verde azulado los metadatos, que acompañan a todo lo demás.
:::
...
[^1]: Uso la traducción tabular de el término inglés _relation_ que se refiere a la estructura de los datos y la traducción a relacional puede dar a entender una gestión relacional que no es tal en los sistemas que ideó Codd.