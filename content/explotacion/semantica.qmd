...
Para nuestro stack hay dos respuestas defendibles. Si la exploración se hace con [Rill](../appendices/rill.qmd), la capa semántica va incluida y consulta directamente el lakehouse. Si el consumidor va a ser más de uno, el sitio correcto es junto a las transformaciones, en el propio dbt, que es de donde las lee [Lightdash](bi.qmd#lightdash). Empecemos por la primera, que es la más corta de enseñar. Una métrica se declara así:
...
Una capa semántica cambia el planteamiento: en lugar de dar al agente las tablas y confiar, se le da un conjunto acotado de medidas y dimensiones con sus definiciones. El agente elige entre métricas ya validadas en lugar de inventarse la agregación. Por eso prácticamente todas las herramientas de esta categoría han incorporado un servidor MCP, y por eso [Rill](../appendices/rill.qmd) se describe a sí mismo hoy como una herramienta de BI *para humanos y agentes*.

Es, mirado con perspectiva, el mismo argumento del [catálogo de gobierno](../appendices/metadatado.qmd): lo que hace útil un dato es el contexto que lo acompaña. Este consumidor tiene [capítulo propio](ai.qmd) más adelante, porque da bastante de sí.
...
Para resolverlo nació **OSI** (_Open Semantic Interchange_), un formato neutral para definir métricas impulsado inicialmente por dbt Labs y Snowflake, que en junio de 2026 entró en la incubadora de Apache con el nombre de [Apache Ossie](https://ossie.apache.org/). Ya existen conversores para MetricFlow, GoodData, Salesforce y Apache Polaris.
